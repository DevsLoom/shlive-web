...
import CoinSales from '../../../../components/Coins/CoinSales';
import ListingTable, {
  TableCell,
...
const SalesCoin = () => {
  ...
  return (
    <>
      <Modal opened={opened} onClose={close} title="Add Sale Coin" centered>
      ...
      <ListingTable
        ...
        body={data?.data?.map((item, i) => (
          <TableTr className="bg-white" key={i}>
            ...
            <TableCell>
              {item.saleAt
                ? moment(item.saleAt).format('Do MMM, YYYY hh:mm A')
                : ''}
            </TableCell>
            <TableCell>
